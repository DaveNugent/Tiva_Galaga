...
{
  // ADD CODE
	ACCEL_CS_PORT->DATA &= ~( 1 << ACCEL_CS_PIN);
}
...
int16_t accel_read_x(void)
{
  // ADD CODE
	uint16_t x_value;
	x_value = (accel_reg_read(ACCEL_OUTX_H_XL)<<8);
	x_value |= accel_reg_read(ACCEL_OUTX_L_XL);
	
  return x_value; // Modify to return the x value
...
int16_t accel_read_y(void)
{
  // ADD CODE
	uint16_t y_value;
	y_value = (accel_reg_read(ACCEL_OUTY_H_XL)<<8);
	y_value |= accel_reg_read(ACCEL_OUTY_L_XL);
...
int16_t accel_read_z(void)
{
  // ADD CODE
  uint16_t z_value;
	z_value = (accel_reg_read(ACCEL_OUTZ_H_XL)<<8);
	z_value |= accel_reg_read(ACCEL_OUTZ_L_XL);
